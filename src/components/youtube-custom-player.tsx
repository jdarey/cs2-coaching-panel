...
export function YoutubeCustomPlayer({
  ...
  // Content protection (shared with the raw-embed wrapper): identity
  // watermark, blocked shortcuts, DevTools detection with auto-pause, and the
  // tab-hide warning. Absolute prevention of screen capture is impossible in
  // a browser (OBS, phone camera, hardware capture), but the watermark makes
  // ANY leak traceable to the account it came from.
  ...
  return (
    ...
    >
      ...
      {/* 8b. CONTENT PROTECTION — identity watermark, DevTools blocker and
           capture-warning toast (shared layer, same as the raw-embed
           wrapper). The watermark drifts between spots every 40s so any leak
           is traceable to the account it came from. */}
